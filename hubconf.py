...
def hubert_SSL(
    ...
    r"""HuBERT from `"A Comparison of Discrete and Soft Speech Units for Improved Voice Conversion"`.
    Args:
        pretrained (bool): load pretrained weights into the model.
        progress (bool): show progress bar when downloading model.
    """
    hubert = HubertSSL()
    if pretrained: 
        checkpoint = torch.hub.load_state_dict_from_url( # need to be fixed
            URLS["hubert-soft"],
            progress=progress,
        )
